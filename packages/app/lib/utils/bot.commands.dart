...
Future<void> handleLocalCommands(
  ...
  if (interaction is ApplicationCommandAutocompleteInteraction) {
    ...
    if (action["id"] == command.id.toString()) {
      void appendReplayFallback(String reason) {
      ...
      if (executionMode == 'bdfd_script') {
        ...
        if (compileResult.actions.isEmpty) {
          ...
          appendReplayFallback('No actions generated by BDFD script.');
          return;
        }
        ...
        try {
          ...
          if (_debugReplayCapturing && !replayCapturedByActions) {
            appendReplayFallback('No replay frames were produced.');
          }
      ...
      try {
        ...
        if (_debugReplayCapturing && !replayCapturedByActions) {
          final reason =
              actionsJson.isEmpty
                  ? 'No workflow actions configured for this command.'
                  : 'No replay frames were produced.';
          appendReplayFallback(reason);
        }
