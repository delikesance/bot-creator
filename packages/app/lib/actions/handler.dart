...
String? _resolveScopeContextId({
  ...
  switch (scope) {
    ...
    case 'guildMember':
      return fromVariables('member.id') ?? fromVariables('userId') ?? interactionUserId();
    case 'message':
      return fromVariables('message.id') ?? interactionMessageId();
    default:
      return null;
  }
}
...
dynamic _resolveVariableValuePayload(
  ...
  final valueType =
      resolveValue((payload['valueType'] ?? 'string').toString()).trim().toLowerCase();
  if (valueType == 'number') {
    final numberRaw = resolveValue((payload['numberValue'] ?? '').toString()).trim();
    ...
    try {
      switch (action.type) {
        ...
        case BotCreatorActionType.setScopedVariable:
          ...
          await manager.setScopedVariable(botId, scope, contextId, key, value);
        ...
        case BotCreatorActionType.getScopedVariable:
          ...
          final value =
              await manager.getScopedVariable(botId, scope, contextId, key) ?? '';
          final storeAs =
              resolveValue((action.payload['storeAs'] ?? '$scope.$key').toString()).trim();
        ...
        case BotCreatorActionType.removeScopedVariable:
          ...
          await manager.removeScopedVariable(botId, scope, contextId, key);
